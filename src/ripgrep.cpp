...
std::string run_ripgrep(std::string pattern) {
  boost::process::ipstream output;
  std::string command = std::format("rg -i {} --json", pattern);
  boost::process::child child_process(command, boost::process::std_out > output);
  std::string output_string((std::istreambuf_iterator<char>(output)),std::istreambuf_iterator<char>());
